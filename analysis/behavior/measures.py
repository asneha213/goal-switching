...
from generate import get_experiment_details
...
class SubjectMeasure:
    def __init__(self, subject_id, experiment, model_res=None):
        ...
        self.details = get_experiment_details(experiment)
    ...
    def get_measure_condition(self, measure_name):
        ...
        measure_condition = np.zeros((self.num_conditions, int(self.num_episodes/ self.num_conditions)))
    ...
    def get_task_performance(self):
        ...
        if self.experiment != 3:
            ...
            block = self.blocks[str(11)]

        return total_rewards


    def get_num_goals_experiment3(self):
        num_goals = np.zeros(self.num_conditions)
        counts = [0,0,0]
        for block_num in range(len(self.blocks)):
            ...
            for trial_num in range(self.num_trials):
                ...
                slots = list(trial['slots'].values())
                suits = [int(i/7) for i in slots]
                for i in range(3):
                    if counts[i] == suits[i]:
                        continue
                    elif counts[i] < suits[i]:
                        num_goals[condition] += 1
                        counts[i] = suits[i]
    ...
    def get_retro_valuation_per_count(self):
        ...
        for block_num in range(len(self.blocks)):
            for trial_num in range(self.num_trials):
                ...
                if cardselect == max_slot_token:
                    # if (cardselect != max_prob_token) and (cardselect != max_slot_token):
                    retro_values[condition][slots[max_slot_token] - slots[max_prob_token]] += 1

        return retro_values/ counts

    def get_proportion_divergence(self):
        counts = 0
    ...
    def get_goal_actions_array(self):
        ...
        #return choices_p, choices_c, choices_b
    ...
    def get_learning_blocks(self):
        ...
        for block_num in range(len(self.blocks)):
            for trial_num in range(self.num_trials):
                ...
                cardflip = trial['cardselect'][0].lower()

                if cardflip == max_prob_token:
                    if self.experiment == 1:
                        if (trial_num < self.num_trials / 2) and (max_prob == 0.8):
                            learning[0] += 1
                        elif (trial_num >= self.num_trials / 2) and (max_prob == 0.8):
                            learning[1] += 1
                        elif (trial_num < self.num_trials / 2) and (max_prob == 0.7):
                            learning[2] += 1
                        elif (trial_num >= self.num_trials / 2) and (max_prob == 0.7):
                            learning[3] += 1
                        elif (trial_num < self.num_trials / 2) and (max_prob == 0.6):
                            learning[4] += 1
                        elif (trial_num >= self.num_trials / 2) and (max_prob == 0.6):
                            learning[5] += 1
                    elif self.experiment == 2:
                        if (trial_num < 15) and (max_prob == 0.75):
                            learning[0] += 1
                        # elif (trial_num >= 10) and (trial_num < 20) and (max_prob == 0.75):
                        #     learning[1] += 1
                        elif (trial_num >= 15) and (max_prob == 0.75):
                            learning[1] += 1
                        elif (trial_num < 15) and (max_prob == 0.55):
                            learning[2] += 1
                        # elif (trial_num >= 10) and (trial_num < 20) and (max_prob == 0.55):
                        #     learning[4] += 1
                        elif (trial_num >= 15) and (max_prob == 0.55):
                            learning[3] += 1

        return np.array(learning / (self.num_episodes * self.num_trials / (2 * self.num_conditions)))

    def get_action_towards_goal(self):
        choices_blocks = []
        for block_num in range(16):
            reward_trials = self.blocks[str(block_num)]['reward_trials']
            targets = self.blocks[str(block_num)]['targets']
            trial_start = 0
            for i in range(len(reward_trials)):
                target = targets[i]
                trial_end = reward_trials[i]
                actions = np.zeros(60)
                actions[:] = np.nan
                for trial_num in range(trial_start, trial_end + 1):
                    trial = self.blocks[str(block_num)]['trial_' + str(trial_num)]
                    cards = [trial['37'][0].lower(), trial['39'][0].lower()]
                    if target not in cards:
                        continue
                    flip = trial['cardflip'][0].lower()
                    if flip == target:
                        actions[trial_end - trial_num] = 1
                    else:
                        actions[trial_end - trial_num] = 0
                choices_blocks.append(actions)
                trial_start = trial_end + 1
        choices_blocks = np.array(choices_blocks)
        choices_blocks_mean = np.nanmean(choices_blocks, axis=0)
        return choices_blocks_mean

    def get_individual_measure(self, measure_name):

        if measure_name == "performance":
            measure = self.get_task_performance()
        elif measure_name == "learning":
            measure = self.get_learning_blocks()
