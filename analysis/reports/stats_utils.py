...
def get_anova_conditions(experiment, measures):
    ...
    if experiment == 1:
        data_dict = {'subjects': list(range(num_subs)) * 3,
                     ...
                     'switches': list(measures[:, 0]) + list(measures[:, 1]) + list(measures[:, 2])}
    elif experiment == 2:
        data_dict = {'subjects': list(range(num_subs)) * 2, 'condition': [0] * num_subs + [1] * num_subs,
                     'switches': list(measures[:, 0]) + list(measures[:, 1])}
    df = pd.DataFrame(data_dict)
    print(AnovaRM(data=df, depvar='switches',
                  subject='subjects', within=['condition']).fit())


def get_anova_partial_conditions(experiment, measures, i):
    import pandas as pd
    from statsmodels.stats.anova import AnovaRM

    num_subs = len(measures)
    if experiment == 1:
        data_dict = {'subjects': list(range(num_subs)) * 2,
                     'condition': [i] * num_subs + [i+1] * num_subs,
                     'switches': list(measures[:, i]) + list(measures[:, i+1])}
